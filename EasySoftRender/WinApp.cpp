...
bool WinApp::Init()
{
	...
	hBuffBitmap = CreateCompatibleBitmap(screen_hdc, mClientWidth/* * 4*/, mClientHeight/* * 4*/);
	HBITMAP o = static_cast<HBITMAP>(SelectObject(hBuffDC, hBuffBitmap));
	DeleteObject(o);
	renderSys.SetClientRect(mClientWidth/* * 4*/, mClientHeight/* * 4*/);
...
void WinApp::DrawScene()
{
	if (renderSys.IsDirty())
	{
		...
		SetDIBits(hBuffDC, hBuffBitmap, 0, mClientHeight/* * 4*/, bufferPtr, &bitmapinfo, DIB_RGB_COLORS);
		RECT rect;
...
void WinApp::SwapBuff(HDC hdc)
{
	...
	//StretchBlt(hdc, 0, 0, mClientWidth, mClientHeight, hBuffDC, 0, 0, mClientWidth * 4, mClientHeight * 4, SRCCOPY);
}
...
void WinApp::SetBitmapInfo()
{
	...
	bitmapinfo.bmiHeader.biHeight = mClientHeight/* * 4*/;
	bitmapinfo.bmiHeader.biPlanes = 1;
	bitmapinfo.bmiHeader.biSizeImage = 0;
	bitmapinfo.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
	bitmapinfo.bmiHeader.biWidth = mClientWidth/* * 4*/;
}
